...
// generic types "T" or "M" we use instead of interfaces 
// when you need to maintain type safety and avoid the
// overhead of type assertions.
func decodeEntities[T any, M any](ctx context.Context, cursor *mongo.Cursor, newEntity func() *T, newModel func() *M) ([]*T, error) {
	...
	for cursor.Next(ctx) {
		...
		entity := newEntity()
		// .Elem() dereferences a variable...
		// meaning a variable was a reference and now we do not need it 
